    ...
    def create(self, *args: object, **kwargs: object) -> object:
        ...
        try:
            if config.mode.checks_input:
                decision = self._guard.check_input(
            ...
            if config.rag is not None and config.rag.mode.value != "disabled":
                documents = retrieve_documents(request, config.rag, trace)
            ...
            response = self._resource.create(*args, **final_kwargs)
            ...
            if config.mode.checks_output:
                decision = self._guard.check_output(
                    GuardOutputContext(
            ...
            if config.rag is not None and config.rag.mode.verifies:
                grounding = self._guard.check_grounding(
                    request=request,
                    response=guarded_response,
                    documents=documents,
                )
